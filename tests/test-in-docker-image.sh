...
yum -y install PyYAML python-paramiko python-jinja2 python-httplib2 rpm-build make python2-devel asciidoc file ethtool iproute iptables less lzo which groff-base libunistring ||(echo "Could not install some deps" && exit 2 )
